...
/*
 * Modified by Yewang Chen<ywchen@hqu.edu.cn;nalandoo@gmail.com>
 * License: GPL v1
 * This software may be modified and distributed under the terms
 * of license.
 *
...
public:
  ...
  static Vector<T> rand_vector(const unsigned int size);
  inline void set(const T* a, const unsigned int n);
  inline Vector<T> extract(const std::set<unsigned int>& indexes) const;
  inline Vector<T> extract(const Vector<int>& indexes) const;
  inline T& operator[](const unsigned int& i); //i-th element
  inline const T& operator[](const unsigned int& i) const;
  inline T* get_matrix_raw_data() const;

  inline Vector<T> product(Matrix<T>& mat);

  inline T product(Vector<T>& vec);
  inline T inner_poduct();
  ...
  inline Vector<T>& operator/=(const T& a);
  inline Vector<T>& operator^=(const T& a);
  inline Vector<int> findEqual(T val);
  inline Vector<int> findLess (T val);
  inline Vector<int> findLessAndEqual(T val);
  inline Vector<int> findLarge (T val);
  inline Vector<int> findLargeAndEqual(T val);
  inline void destroy();
private:
...
template <typename T>
void Vector<T>::print(const char* info)
{
    std::cout<<info<<"[";
...
template <typename T>
 Vector<T> Vector<T>::rand_vector(const unsigned int size)
{
...
template <typename T>
inline T Vector<T>::product(Vector<T>& vec){

    if (vec.size()!=this->n)
        throw std::logic_error("The dimension is not compatible to product.");

    T result=0;
    for (unsigned int i = 0; i < this->n; i++){
       result+= vec[i]* this->v[i];
    }
    return result;
}

template <typename T>
inline T Vector<T>::inner_poduct(){
...
template <typename T>
inline Vector<T> Vector<T>::product(Matrix<T>& mat)
{
    int rows=mat.nrows();
    int cols=mat.ncols();
    if (cols!=this->n)
...
template <typename T>
inline Vector<T> Vector<T>::extract(const Vector<int>& indexes) const
{
  Vector<T> tmp(indexes.size());
  int i = 0;
...
template <typename T>
inline T* Vector<T>::get_matrix_raw_data() const
{
    return this->v;
...
template <typename T>
inline Vector<int> Vector<T>::findEqual(T val){
    int num=this->size();
    Vector<int> tmp (num);
    int count=0;
    for (int i=0;i<this->size(); i++){
...
template <typename T>
inline Vector<int> Vector<T>::findLess (T val){
    int num=this->size();
    Vector<int> tmp (num);
    int count=0;
    for (int i=0;i<this->size(); i++){
...
template <typename T>
inline Vector<int> Vector<T>::findLessAndEqual(T val){
    int num=this->size();
    Vector<int> tmp (num);
    int count=0;
    for (int i=0;i<this->size(); i++){
...
template <typename T>
inline Vector<int> Vector<T>::findLarge (T val){
    int num=this->size();
    Vector<int> tmp (num);
    int count=0;
    for (int i=0;i<this->size(); i++){
...
template <typename T>
inline Vector<int> Vector<T>::findLargeAndEqual(T val){
    int num=this->size();
    Vector<int> tmp (num);
    int count=0;
    for (int i=0;i<this->size(); i++){
...
template <typename T>
inline unsigned int index_max(T* v, int len)
{
  unsigned int max = 0;
...
template <typename T>
inline unsigned int index_min(T* v, int len)
{
  unsigned int min = 0;
  for (unsigned int i = 1; i < len; i++)
    if (v[i] < v[min])
...
template <typename T>
inline Vector<T> pow(const Vector<T>& v, T a)
{
  Vector<T> tmp(v.size());
  for (unsigned int i = 0; i < v.size(); i++)
    tmp[i] = pow(v[i], a);
...
template <typename T>
inline Vector<T> rank(const Vector<T>& v)
{
  Vector<T> tmp(v);
  Vector<T> tmp_rank(0.0, v.size());
...
public:
  ...
  static Matrix<T> rand_matrix(const unsigned int n, const unsigned int m);
  ...
  inline Vector<T> sum() const;
  inline Matrix<T> extractRows(const std::set<unsigned int>& indexes) const;
  inline Matrix<T> extractRows(const Vector<int>& indexes) const;
  inline Matrix<T> transform_matrix() const;

  inline Matrix<T> extractColumns(const std::set<unsigned int>& indexes) const;
  inline Matrix<T> extractColumns(const Vector<int>& indexes) const;
  ...
  inline void setRow(const unsigned int i, T* vec);
  inline void setRow(const unsigned int i, const Vector<T>& a);
  ...
  inline void setRows(Vector<int>& indexes, const Matrix<T>& m);
  ...
  Vector<T> product(Vector<T>& vec);
  inline T* get_matrix_raw_data();
  inline T* get_matrix_raw_row_data(int row_id);
  ...
  inline void set_val(T* a, const unsigned int n, const unsigned int m);

  void write_data_into_vectors(Vector<Vector<T> >& vec);
...
//write the data into vec
template <class T>
void Matrix<T>::write_data_into_vectors(Vector<Vector<T> >& vec){
     int dim=this->ncols();
     int data_len=this->nrows();
     vec.resize(data_len);
     Vector<T>* tmp=vec.get_matrix_raw_data();
     for (int i=0;i<data_len;i++){
        tmp[i].set(this->get_matrix_raw_row_data(i),dim);
     }
  }
...
template <class T>
void Matrix<T>::set_val(T* a, unsigned int n, unsigned int m)
{
  register unsigned int i, j;
  this->n = n; this->m = m;
  v= new T*[n];
...
template <typename T>
 Matrix<T> Matrix<T>::rand_matrix(const unsigned int n, const unsigned int m){
    Matrix<T> result(n,m);
    for (int i=0; i<n;i++){
        for (int j=0; j<m;j++){
            result[i][j]=rand();
...
template <typename T>
inline Matrix<T> Matrix<T>::transform_matrix() const
{
    register int i,j;
...
template <typename T>
inline Matrix<T> Matrix<T>::extractRows(const Vector<int>& indexes) const
{
  Matrix<T> tmp(indexes.size(), m);
  register int i = 0, j;
...
template <typename T>
inline Matrix<T> Matrix<T>::extractColumns(const Vector<int>& indexes) const
{
  Matrix<T> tmp(n, indexes.size());
  register unsigned int i, j = 0, len=indexes.size();

  for (i=0;i<this->n;i++)
    {
        for (j = 0; j <len; j++)
        {
            tmp[i][j] = v[i][indexes[j]];
        }
    }
  return tmp;
}
...
template <typename T>
inline void Matrix<T>::setRow(const unsigned int i, const Vector<T>& vec)
{
  if (this->m!= vec.size())
       throw std::logic_error("the dimension is not compatible.");

  if (i>= this->n)
       throw std::logic_error("The rows is overflow.");
...
template <typename T>
inline void Matrix<T>::setRow(const unsigned int i, T* vec)
{
  if (i>= this->n)
       throw std::logic_error("The rows is overflow.");
...
template <typename T>
inline void Matrix<T>::setRows(Vector<int>& indexes, const Matrix<T>& m)
{
  unsigned int i = 0;
...
template<typename T>
inline Vector<T> Matrix<T>::sum() const{
...
template <typename T>
inline T* Matrix<T>::get_matrix_raw_data(){
    return v[0];
}

template <typename T>
inline T* Matrix<T>::get_matrix_raw_row_data(int row_id){
    return v[row_id];
}

template <typename T>
Vector<T> Matrix<T>::product (Vector<T>& vec)
{
    if (vec.size()!=m)
        throw std::logic_error("Operator*=: vector has different sizes");

    Vector<T> result (n);
    for (unsigned int i = 0; i < n; i++){
        T tmp=0;
        for (int j=0;j<m;j++){
            tmp+= this->v[i][j]*vec[j];
        }
        result[i] =tmp ;
    }
    return result;
...
Matrix<T> pinv(const Matrix<T>& A)
{
  ...
  for (unsigned int i = 0; i < A.nrows(); i++)
    {
      e.reset(i);
      tmp.setColumn(i, dot_prod(dot_prod(dot_prod(V, Matrix<T>(DIAG, 1.0 / W, 0.0, W.size(), W.size())), t(U)), e));
    }
